...
class BaseEnv(gym.Env):
    physics_fps = 120
    decimation = 5  # physics steps per control step; effective control rate = 120/5 = 24 Hz
    ...
    def __init__(self,
        ...
        if video_cfg:
            ...
            # Write video on process exit so it is saved even if close() is never called
        ...
        if log_path:
            ...
            # Same atexit pattern as video — guarantees the log is flushed on exit
            atexit.register(self._save_steps)


    def get_current_frame(self):
        # Recreate renderer each call; avoids stale scene state when called from MCP subprocess
    ...
    def render(self):
        ...
        if self.render_mode == "rgb_array":
            ...
            if self.render_realtime:
                # slowdown the render speed for human viewing
    ...
    def step(self, action: np.ndarray):
        ...
        # Run multiple physics tiks per control step (decimation)
